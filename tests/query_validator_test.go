...
import (
	"github.com/Mamoru-Foundation/mamoru-core-go/query_validator"
...
func BenchmarkSimpleValidation(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_ = query_validator.ValidateSql(query_validator.ChainSui, "SELECT * FROM transactions", nil, nil)
	}
}

func TestValidateValidSuiExpression(t *testing.T) {
	err := query_validator.ValidateSql(query_validator.ChainSui, "SELECT 1 FROM transactions t WHERE starts_with(t.digest, '0x1_this_is_an_example_query')", nil, nil)

	assert.Nil(t, err)
}

func TestValidateInvalidSuiExpression(t *testing.T) {
	err := query_validator.ValidateSql(query_validator.ChainSui, "SELECT * FROM dummy", nil, nil)

	assert.NotNil(t, err)
}

func TestValidateValidEvmExpression(t *testing.T) {
	err := query_validator.ValidateSql(query_validator.ChainEvm, "SELECT * FROM blocks", nil, nil)

	assert.Nil(t, err)
}

func TestValidateInvalidEvmExpression(t *testing.T) {
	err := query_validator.ValidateSql(query_validator.ChainEvm, "SELECT * FROM dummy", nil, nil)

	assert.NotNil(t, err)
}

func TestValidateValidAptosExpression(t *testing.T) {
	err := query_validator.ValidateSql(query_validator.ChainAptos, "SELECT * FROM blocks", nil, nil)

	assert.Nil(t, err)
}

func TestValidateInvalidAptosExpression(t *testing.T) {
	err := query_validator.ValidateSql(query_validator.ChainAptos, "SELECT * FROM dummy", nil, nil)
...
func TestValidateSqlRendersMissingParameter(t *testing.T) {
	err := query_validator.ValidateSqlRenders("SELECT * FROM blocks WHERE height = {{ height }}", map[string]string{}, nil)

	assert.EqualError(t, err, "RenderSql(\n"+
		"    RenderError {\n"+
		"        desc: \"Variable \\\"height\\\" not found in strict mode.\",\n"+
		"        template_name: None,\n"+
		"        line_no: Some(\n"+
		"            1,\n"+
		"        ),\n"+
...
func TestValidateSqlRendersSyntaxError(t *testing.T) {
	err := query_validator.ValidateSqlRenders("SELECT * FROM blocks WHERE height = {{ height }", map[string]string{
		"height": "123",
	}, nil)
	...
	assert.EqualError(t, err, "RenderSql(\n"+
		...
		"                ),\n"+
		"                segment: Some(\n"+
		"                    \"   0 | SELECT * FROM blocks WHERE height = {{ height }\\n     |-----------------------------------------------\\n\",\n"+
		"                ),\n"+
		"            },\n"+
		"        ),\n"+
		"        unimplemented: false,\n"+
...
func TestValidatePassVersions(t *testing.T) {
	err := query_validator.ValidateSql(query_validator.ChainEvm, "SELECT * FROM dummy", nil, map[string]string{
		"mamoru": "1.0.0",
	})
...
func TestValidateInvalidVersion(t *testing.T) {
	...
	assert.EqualError(t, err, "error validating versions: InvalidVersion(\n"+
		"    \"1\",\n"+
		")")
}

func TestValidateValidAssemblyScript(t *testing.T) {
	err := query_validator.ValidateAssemblyScript(query_validator.ChainEvm, ValidAssemblyScript[:], nil)

	assert.Nil(t, err)
}

var InvalidAssemblyScript = [...]byte{
